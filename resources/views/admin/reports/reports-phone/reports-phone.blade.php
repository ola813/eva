...
@section('content')
<div class="row">
<form action="{{route('Search_Report_Phone')}}" method="POST" role="search" autocomplete="off" enctype="multipart/form-data">
    {{ csrf_field() }}
...
<div class="card-body">
                <div class="table-responsive">
                    @if (isset($orderReports))
                        ...
                        @if ($i == 0)
                        <button class="btn btn-success m-r-10"  disabled onclick="tablesToExcel(['example'], ['OrderReport'], 'تفارير فواتير الارضي.xls', 'Excel')">{{trans('electronic/electronic.Export to Excel')}}</button>
                        @else
                        <button class="btn btn-success m-r-10"   onclick="tablesToExcel(['example'], ['OrderReport'], 'تقارير فواتير الارضي.xls', 'Excel')">{{trans('electronic/electronic.Export to Excel')}}</button>
                        @endif
                        @endif
