...
@section('content')
<div class="row">
<form action="{{route('Search_Report_internet')}}" method="POST" role="search" autocomplete="off" enctype="multipart/form-data">
    {{ csrf_field() }}
...
<div class="card-body">
                <div class="table-responsive">
                    @if (isset($orderReports))
                        <table id="example" class="table key-buttons text-md-nowrap" style=" text-align: center">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border-bottom-0">اسم صاحب الفاتورة</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($orderReports as $orderReport)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$orderReport->number}}</td>
                                        <td>{{$orderReport->full_name}}</td>
                        ...
                        @if ($i == 0)
                        <button class="btn btn-success m-r-10"  disabled onclick="tablesToExcel(['example'], ['OrderReport'], 'تقارير فواتير الانترنت.xls', 'Excel')">{{trans('electronic/electronic.Export to Excel')}}</button>
                        @else
                        <button class="btn btn-success m-r-10"   onclick="tablesToExcel(['example'], ['OrderReport'], 'تقارير فواتير الانترنت.xls', 'Excel')">{{trans('electronic/electronic.Export to Excel')}}</button>
                        @endif
                        @endif
