...
@section('content')
<div class="row">
<form action="{{route('Search_Report_Balance')}}" method="POST" role="search" autocomplete="off" enctype="multipart/form-data">
    {{ csrf_field() }}
...
<<br>
                ...
                <!-- <div class="row">
                    <div class="col-sm-1 col-md-1"> -->
                        <button class="btn btn-danger btn-block m-b-20 m-r-10">بحث</button>
                    <!-- </div>
                </div> -->
...
<div class="card-body">
                <div class="table-responsive">
                    @if (isset($orderReports))
                        ...
                        @if ($i == 0)
                        <button class="btn btn-success m-r-10"  disabled onclick="tablesToExcel(['example'], ['OrderReport'], 'تقارير  تحويل الرصيد.xls', 'Excel')">{{trans('electronic/electronic.Export to Excel')}}</button>
                        @else
                        <button class="btn btn-success m-r-10"   onclick="tablesToExcel(['example'], ['OrderReport'], 'تقارير تحويل الرصيد.xls', 'Excel')">{{trans('electronic/electronic.Export to Excel')}}</button>
                        @endif
                        @endif
